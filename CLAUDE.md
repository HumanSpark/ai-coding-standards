...
15. **Image Size Limit for Visual Review (2026-04-08):** Added critical image size limit rule: never create or read images >= 2000px on either dimension (breaks conversation context). Updated visual-review skill with dedicated section, fixed contact sheet code to use 1900px safety margin, fixed before/after comparison to resize halves. Added matching rule to user-level CLAUDE.md Development section. Evidence: observed session crashes when large screenshots or contact sheets exceeded Claude's image processing limits.
16. **Token Efficiency Configuration (2026-04-15):** Switched user-level `model` to `opusplan` (Opus for Plan Mode, Sonnet for execution) and added env block pinning subagents to Haiku 4.5 and bumping `BASH_MAX_OUTPUT_LENGTH` to 50k. Added `claudeMdExcludes` for `project-template/**` and `archive/**` to prevent stray CLAUDE.md loads. Expanded `permissions.deny` (user-level + project-template) with build artefact dirs (node_modules, dist, build, __pycache__, .venv, *.egg-info) to block exploratory reads. Extracted three session-level meta-rules (context decay, edit verification, tool output truncation, sub-agent discipline) from user-level CLAUDE.md into new `user-level/rules/development-discipline.md`, freeing ~400 tokens from every-session context. Evidence: `/context` inspection showed user-level CLAUDE.md over budget (3.2k / 3.0k) and running Opus-high everywhere with no subagent model override.
19. **Stub Justification Enforcement (2026-04-23):** Rule 4.9 is now enforced deterministically by the user-level pre-commit hook, not just documented. Hook scans staged changes for two cases: (a) new spec files under `docs/plans/*.md` missing a `Why:` or `Trigger:` line, and (b) newly-added `TODO:`/`STUB:`/`FIXME:` code comments without `Why:` and `Trigger:` within a 5-line window in the same hunk. Template docs, reference doc, CLAUDE.md files, tests, and fixtures are excluded from (b). Updated both `docs/SPEC-TEMPLATE.md` and `project-template/docs/SPEC-TEMPLATE.md` with Why/Trigger placeholders so generated specs start compliant. Override via `--no-verify` for genuine exceptions. Evidence: documented-but-not-enforced rules decay; matches the same deterministic-first pattern applied to secrets and sensitive filenames in April 2026.
18. **Stub Justification Standard (2026-04-22):** Added Rule 4.9 to reference doc - every stub, `TODO:`, task-register entry, and backlog note MUST include `Why:` and `Trigger:` lines at creation. Without a Why, don't create the stub; items missing Why + trigger during a sweep are candidates for immediate close. Updated user-level CLAUDE.md Code Style (Placeholders rule) and Do Not list. Evidence: session observation that stubs accumulate faster than they resolve because individual stubs look cheap; three-day-old stubs become unactionable when trigger context is lost (sparkcore vision A4, image-DPI gate, running-header gate all hit this). Fix is a stricter creation bar, not better memory.
17. **Python Deny Expansion + Nosync Sentinel (2026-04-15):** Added `**/*.pyc`, `**/.mypy_cache/**`, `**/.ruff_cache/**` to `permissions.deny` in `project-template/.claude/settings.json` (covers `.pyc` files outside `__pycache__` and the two Python linter caches that grow fast in active projects). Cascaded additively to 14 managed projects via `setup.sh`. Added `.claude/.nosync` sentinel support in `discover_projects()`: any directory with `.claude/.nosync` is skipped during auto-discovery, so non-Python workspaces (e.g. `~/books/`) with their own `.claude/skills/` do not receive coding-project boilerplate. Evidence: auto-discovery deployed Python agents, rules, and skills into the books writing workspace after it gained a `.claude/` directory for workspace-scoped writing skills; sentinel gives an opt-out without hard-coding paths.
...
| user-level/rules/development-discipline.md | Session meta-rules: context decay, edit verification, sub-agent discipline |
